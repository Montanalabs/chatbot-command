...
- **Sink / capability:** `grant runChat` → `using runChat { privileged { runChat(d) } }`
...
- `chatbot-command_safe.eide` — the correct design.
- `chatbot-command_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
